...
# Runs the frontend
function qs-front() {
    cd $HOME/dev/QuickSnackFront/front
    npm run dev
}

# Runs the backend
function qs-back() {
    cd $HOME/dev/QuickSnackBack/back
    python3 manage.py runserver
}

# Pulls the changes on every project
function qs-pull() {
    printf  "${CYAN}pulling QuickSnackBack${NC}\n"
    cd $HOME/dev/QuickSnackBack && git status
    git fetch
    git pull
    printf  "${CYAN}pulling QuickSnackFront${NC}\n"
    cd $HOME/dev/QuickSnackFront && git status
    git fetch
    git pull
}
...
function qs-push() {
    printf  "${CYAN}pushing to QuickSnackBack${NC}\n"
    cd $HOME/dev/QuickSnackBack && git status && git add . && git commit -m "Updated: `date +'%d-%m-%Y %H:%M:%S'`" && git push
    printf  "${CYAN}puhsing to QuickSnackFront${NC}\n"
    cd $HOME/dev/QuickSnackFront && git status && git add . && git commit -m "Updated: `date +'%d-%m-%Y %H:%M:%S'`" && git push
}
...
function qs-projects() {
    ...
    cd $HOME/dev
    git clone git@gitlab.com:GregoryHue/QuickSnackBack.git
    cd QuickSnackBack/back
    python3 manage.py makemigrations && python3 manage.py migrate
    
    cd $HOME/dev
    git clone https://git@gitlab.com:GregoryHue/QuickSnackFront.git
    cd QuickSnackFront/front
    npm install
}
